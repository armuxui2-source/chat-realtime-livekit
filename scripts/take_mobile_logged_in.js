const { chromium } = require("playwright");
const path = require("path");

const ARTIFACT_DIR =
  "C:\\Users\\armyn\\.gemini\\antigravity-ide\\brain\\c41c62cc-120d-42ff-9c07-c23b52b92fd8";

(async () => {
  const browser = await chromium.launch({ headless: true });
  const context = await browser.newContext({
    viewport: { width: 390, height: 844 }, // iPhone 14 / Mobile standard
  });
  const page = await context.newPage();
  await page.goto("http://localhost:3000", { timeout: 30000 });
  await page.waitForTimeout(1000);

  // Switch to quick test account & select Sarah Miller
  try {
    await page.click('button:has-text("ทดสอบ")', { timeout: 2000 });
    await page.waitForTimeout(400);
    await page.click('button:has-text("Sarah Miller")', { timeout: 3000 });
    await page.waitForTimeout(1200);
  } catch (e) {}

  const mobileLoggedInPath = path.join(
    ARTIFACT_DIR,
    "mobile_logged_in_bottom_nav.png"
  );
  await page.screenshot({ path: mobileLoggedInPath, fullPage: false });
  console.log("SUCCESS_MOBILE_LOGGED_IN:", mobileLoggedInPath);

  await browser.close();
})();
