...
const workingPrinciplesContent = `# 🎯 Master Partnership & Working Principles (กฎเหล็กประจำตัวในการทำงานร่วมกัน)
...
// 1. Write to Global Rules Root
const globalRulesDir = "C:\\\\Users\\\\armyn\\\\.gemini\\\\config\\\\rules";
fs.mkdirSync(globalRulesDir, { recursive: true });
fs.writeFileSync(path.join(globalRulesDir, "working-principles.md"), workingPrinciplesContent, "utf8");

// 2. Write to Global GEMINI.md in config root
const globalConfigDir = "C:\\\\Users\\\\armyn\\\\.gemini\\\\config";
fs.writeFileSync(path.join(globalConfigDir, "GEMINI.md"), workingPrinciplesContent, "utf8");

// 3. Write to Workspace local .agents/
const localRulesDir = path.join(__dirname, "..", ".agents", "rules");
fs.mkdirSync(localRulesDir, { recursive: true });
fs.writeFileSync(path.join(localRulesDir, "working-principles.md"), workingPrinciplesContent, "utf8");

const localAgentsMd = path.join(__dirname, "..", "AGENTS.md");
fs.writeFileSync(localAgentsMd, workingPrinciplesContent, "utf8");

console.log("GLOBAL_AND_LOCAL_WORKING_PRINCIPLES_REGISTERED_SUCCESSFULLY");
